...
impl<F: Field> RLPListGadget<F> {
    ...
    pub(crate) fn rlc_rlp_only(&self, r: &Expression<F>) -> (Expression<F>, Expression<F>) {
        circuit!([meta, _cb!()], {
            matchx! {
                self.is_short() => self.bytes[..1].be_rlc(r),
                self.is_long() => self.bytes[..2].be_rlc(r),
                self.is_very_long() => self.bytes[..3].be_rlc(r),
            }
        })
    }
...
impl RLPListWitness {
    ...
    pub(crate) fn rlc_rlp<F: Field>(&self, r: F) -> F {
        self.bytes.be_rlc_value(r).0
    }

    /// Returns the rlc of the RLP bytes
    pub(crate) fn rlc_rlp_only<F: Field>(&self, r: F) -> (F, F) {
        matchw! {
            self.is_short() => self.bytes[..1].be_rlc_value(r),
            self.is_long() => self.bytes[..2].be_rlc_value(r),
            self.is_very_long() => self.bytes[..3].be_rlc_value(r),
        }
    }
}

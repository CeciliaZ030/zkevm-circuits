...
//         cb.compress_with_local_condition(
//             &format!("{:?} write #{:?}", self.tag, self.writes.1), 
//             values,
//             C::default(),
//             Some(self.query_write()),
//             true
//         );
//         self.table_conditions.push((cb.region_id, cb.get_condition_expr()));
//         key
//     }
...
//         let values = self.prepend_key(key.clone(), values);
//         cb.compress_with_local_condition(
//             &format!("{:?} read #{:?}", self.tag, self.reads.1), 
//             values,
//             C::default(),
